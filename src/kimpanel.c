...
static void KimpanelRegisterComplexStatus(void *arg,
                                          FcitxUIComplexStatus *status);
static void KimpanelUpdateComplexStatus(void *arg,
                                        FcitxUIComplexStatus *status);
...
static void KimpanelOwnerChanged(void *user_data, void *arg,
                                 const char *serviceName, const char *oldName,
                                 const char *newName);
...
static void KimSetSpotRect(FcitxKimpanelUI *kimpanel, int x, int y, int w,
                           int h, boolean relative);
static void KimShowLookupTable(FcitxKimpanelUI *kimpanel, boolean toShow);
static void KimUpdateLookupTable(FcitxKimpanelUI *kimpanel, char *labels[],
                                 int nLabel, char *texts[], int nText,
                                 boolean has_prev, boolean has_next);
static void KimSetLookupTable(FcitxKimpanelUI *kimpanel, char *labels[],
                              int nLabel, char *texts[], int nText,
...
static void KimRegisterProperties(FcitxKimpanelUI *kimpanel, char *props[],
                                  int n);
static void KimUpdateProperty(FcitxKimpanelUI *kimpanel, char *prop);
static DBusHandlerResult KimpanelDBusEventHandler(DBusConnection *connection,
                                                  DBusMessage *message,
                                                  void *user_data);
static DBusHandlerResult KimpanelDBusFilter(DBusConnection *connection,
                                            DBusMessage *message,
                                            void *user_data);
...
static char *ComplexStatus2String(FcitxInstance *instance,
                                  FcitxUIComplexStatus *status);
...
FCITX_DEFINE_PLUGIN(fcitx_kimpanel_ui, ui,
                    FcitxUI) = {KimpanelCreate,
...
void *KimpanelCreate(FcitxInstance *instance) {
    ...
    do {
        ...
        dbus_bus_add_match(kimpanel->conn,
                           "type='signal',sender='org.kde.impanel',interface='"
                           "org.kde.impanel'",
                           &err);
        ...
        if (!dbus_connection_add_filter(kimpanel->conn, KimpanelDBusFilter,
                                        kimpanel, NULL)) {
        ...
        DBusObjectPathVTable vtable = {
            NULL, &KimpanelDBusEventHandler, NULL, NULL, NULL, NULL};

        dbus_connection_register_object_path(
            kimpanel->conn, FCITX_KIMPANEL_PATH, &vtable, kimpanel);
        ...
        dbus_message_append_args(message, DBUS_TYPE_STRING,
                                 &kimpanelServiceName, DBUS_TYPE_INVALID);

        DBusPendingCall *call = NULL;
        dbus_bool_t reply = dbus_connection_send_with_reply(
            kimpanel->conn, message, &call, DBUS_TIMEOUT_USE_DEFAULT);
        if (reply == TRUE) {
            dbus_pending_call_set_notify(call, KimpanelServiceExistCallback,
                                         kimpanel, NULL);
...
void KimpanelRegisterAllStatus(FcitxKimpanelUI *kimpanel) {
    ...
    char **prop = fcitx_utils_malloc0(
        sizeof(char *) * (2 + utarray_len(uistats) + utarray_len(uicompstats)));
...
void KimpanelOnInputFocus(void *arg) {
    FcitxKimpanelUI *kimpanel = (FcitxKimpanelUI *)arg;
    KimEnable(kimpanel,
              (FcitxInstanceGetCurrentStatev2(kimpanel->owner) == IS_ACTIVE));
    KimpanelSetIMStatus(kimpanel);
}
...
void KimpanelMoveInputWindow(void *arg) {
    ...
    if (kimpanel->version == 1)
        ...
        KimSetSpotRect(kimpanel, x, y, w, h,
                       !!(ic->contextCaps & CAPACITY_RELATIVE_CURSOR_RECT));
}
...
char *Status2String(FcitxInstance *instance, FcitxUIStatus *status) {
    ...
    fcitx_utils_alloc_cat_str(
        result, "/Fcitx/", status->name, ":", status->shortDescription,
        ":fcitx-", status->name,
...
char *ComplexStatus2String(FcitxInstance *instance,
                           FcitxUIComplexStatus *status) {
...
void KimpanelShowInputWindow(void *arg) {
    ...
    kimpanel->iCursorPos = FcitxUINewMessageToOldStyleMessage(
        instance, kimpanel->messageUp, kimpanel->messageDown);
    ...
    nTotaltexts = FcitxCandidateWordGetListSize(candList);
    //兼容平板项目

    if (n) {
        for (i = 0; i < n; i++) {
            FcitxLog(DEBUG, "Type: %d Text: %s",
                     FcitxMessagesGetMessageType(messageDown, i),
            ...
            if (FcitxMessagesGetMessageType(messageDown, i) == MSG_INDEX) {
                ...
                char *needfree = FcitxInstanceProcessOutputFilter(
                    instance, FcitxMessagesGetMessageString(messageDown, i));
                char *msgstr;
                if (needfree)
                    msgstr = needfree;
                else
                    msgstr =
                        strdup(FcitxMessagesGetMessageString(messageDown, i));
                ...
                if (needfree)
                    free(needfree);
                if (FcitxMessagesGetMessageType(messageDown, i) ==
                    MSG_FIRSTCAND)
                    pos = nTexts;
            }
        }
        ...
        if (nTexts == 0) {
            ...
            if (kimpanel->hasSetLookupTable)
                KimSetLookupTable(kimpanel, label, nLabels, text, nTexts,
                                  hasPrev, hasNext, pos, layout);
            else if (kimpanel->hasAllCandidates)
                KimAllCandidates(kimpanel, totaltext, nTotaltexts);
            else
                KimUpdateLookupTable(kimpanel, label, nLabels, text, nTexts,
                                     hasPrev, hasNext);
    ...
    if (n) {
        for (i = 0; i < n; i++) {
            ...
            char *needfree = FcitxInstanceProcessOutputFilter(
                instance, FcitxMessagesGetMessageString(messageUp, i));
            char *msgstr;
            if (needfree)
                msgstr = needfree;
            else
...
DBusHandlerResult KimpanelDBusFilter(DBusConnection *connection,
    ...
    if (dbus_message_is_signal(msg, "org.kde.impanel", "MovePreeditCaret")) {
        ...
        dbus_message_get_args(msg, &error, DBUS_TYPE_INT32, &int0,
                              DBUS_TYPE_INVALID);
        ...
        if (FcitxCandidateWordPageCount(
            ...
            FcitxCandidateWordGoPrevPage(
                FcitxInputStateGetCandidateList(input));
            FcitxInstanceProcessInputReturnValue(instance,
        ...
        DBusError error;
        dbus_error_init(&error);
        if (dbus_message_get_args(msg, &error, DBUS_TYPE_STRING, &s0,
                                  DBUS_TYPE_INVALID)) {
            if (strlen(s0) > strlen("/Fcitx/")) {
                ...
                if (strcmp("logo", s0) == 0) {
                    ...
                    fcitx_utils_alloc_cat_str(prop[0],
                                              "/Fcitx/logo/toggle:", trans_str,
                                              "::", trans_str);
                    trans_str = _("Restart");
                    fcitx_utils_alloc_cat_str(prop[1],
                                              "/Fcitx/logo/restart:", trans_str,
                                              ":view-refresh:", trans_str);
                    ...
                    for (pim = (FcitxIM *)utarray_front(imes); pim != NULL;
                    ...
                    if ((pos = strchr(s0, '/'))) {
                        ...
                        FcitxUIMenu *menup =
                            FcitxUIGetMenuByStatusName(instance, statusName);
                        free(statusName);
                        pos++;
                        int index = 0;
                        sscanf(pos, "%d", &index);
                        ...
                        FcitxUIMenu *menu =
                            FcitxUIGetMenuByStatusName(instance, s0);
                        if (menu) {
        ...
        KimpanelRegisterAllStatus(kimpanel);
        return DBUS_HANDLER_RESULT_HANDLED;
    } else if (dbus_message_is_signal(msg, "org.kde.impanel2",
                                      "PanelCreated2")) {
...
void KimpanelOwnerChanged(void *user_data, void *arg, const char *serviceName,
...
void KimExecDialog(FcitxKimpanelUI *kimpanel, char *prop) {
    ...
    msg = dbus_message_new_signal(
        FCITX_KIMPANEL_PATH,      // object name of the signal
    ...
    if (dbus_message_append_args(msg, DBUS_TYPE_STRING, &prop,
                                 DBUS_TYPE_INVALID)) {
...
void KimUpdateLookupTable(FcitxKimpanelUI *kimpanel, char *labels[], int nLabel,
                          char *texts[], int nText, boolean has_prev,
                          boolean has_next) {
    int i;
    dbus_uint32_t serial =
        0; // unique number to associate replies with requests
    ...
    for (i = 0; i < nLabel; i++) {
        if (!dbus_message_iter_append_basic(&subAttrs, DBUS_TYPE_STRING,
                                            &attr)) {
            FcitxLog(DEBUG, "Out Of Memory!");
        }
    }
...
void KimSetLookupTable(FcitxKimpanelUI *kimpanel, char *labels[], int nLabel,
                       char *texts[], int nText, boolean has_prev,
                       boolean has_next, int cursor, int layout) {
    int i;
    dbus_uint32_t serial =
        0; // unique number to associate replies with requests
    DBusMessage *msg;
    ...
    // create a signal and check for errors
    msg = dbus_message_new_method_call("org.kde.impanel", "/org/kde/impanel",
                                       "org.kde.impanel2",
                           ...
                           int nText)
{
    ...
    dbus_message_iter_append_basic(&args, DBUS_TYPE_INT32, &nText);
...
void KimSetSpotRect(FcitxKimpanelUI *kimpanel, int x, int y, int w, int h,
                    boolean relative) {
    if (kimpanel->lastUpdateX == x && kimpanel->lastUpdateY == y &&
        kimpanel->lastUpdateW == w && kimpanel->lastUpdateH == h)
        return;
    kimpanel->lastUpdateX = x;
    kimpanel->lastUpdateY = y;
    ...
    msg = dbus_message_new_method_call(
        ...
        useRelative ? "SetRelativeSpotRect"
    ...
    if (!dbus_message_append_args(msg, DBUS_TYPE_INT32, &x, DBUS_TYPE_INT32, &y,
...
void KimpanelIntrospect(FcitxKimpanelUI *kimpanel) {
    ...
    DBusMessage *message = dbus_message_new_method_call(
        kimpanelServiceName, "/org/kde/impanel", DBUS_INTERFACE_INTROSPECTABLE,
        "Introspect");
    ...
    dbus_message_unref(message);
    if (reply == TRUE) {
        dbus_pending_call_set_notify(call, KimpanelIntrospectCallback, kimpanel,
                                     NULL);
        dbus_pending_call_unref(call);
    }
    dbus_connection_flush(kimpanel->conn);
    ...
    if (msg) {
        ...
        if (dbus_message_get_args(msg, &error, DBUS_TYPE_STRING, &s,
            ...
            if (strstr(s, "org.kde.impanel2")) {
                ...
                if (strstr(s, "KimAllCandidates")) {
                    kimpanel->hasAllCandidates = true;
                }
            }
        }
        dbus_message_unref(msg);
...
void KimpanelSuspend(void *arg) {
    ...
    kimpanel->hasAllCandidates = false;
}
